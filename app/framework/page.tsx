...
export default function Framework(){return <PageShell eyebrow="Calden Terre Impact Framework" title="The Calden Terre Impact Framework (CTIF) is how we take established small and growing businesses from an unclear starting point to practical systems, measurable results, and progress that holds without us in the room."><Reveal><section className="section"><div className="container work-list">{stages.map(([Icon,t,d])=>{const I=Icon as typeof Search;return <article className="work-card" key={String(t)}><div className="icon-badge" style={{marginBottom:0}}><I size={20}/></div><h2 style={{marginTop:16}}>{String(t)}</h2><p className="lead" style={{fontSize:18,marginTop:8}}>{String(d)}</p></article>})}</div></section></Reveal></PageShell>}
